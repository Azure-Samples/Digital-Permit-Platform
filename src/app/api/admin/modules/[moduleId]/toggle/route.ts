...
import { ModuleBuilderError, toggleModule } from "@/lib/modules/registry";
...
export async function POST(
  ...
  try {
    ...
    if (!isTrustedMutationOrigin(req)) return NextResponse.json({ error: "Invalid request origin." }, { status: 403 });
    ...
    if (error instanceof ModuleBuilderError) return NextResponse.json({ error: error.message }, { status: error.status });
